...
fn main() {
    ...
    for num_shares in [2, 4, 8, 16] {
        let mut public_param =
            ss::public_parameters::PublicParams::new(security_parameter, num_shares).unwrap();
        ...
        let client = ss::client::Client::new(
            num_shares,
        ...
        for (dim, shares) in share_of_shares.iter().enumerate() {
            for server_idx in 0..num_shares {
                ...
                agg[server_idx].receive_share(
                    dim,
                    &shares.shares[server_idx],
                    &shares.randomness[server_idx],
                    &shares.commitments[server_idx],
                    &mut public_param.ctx,
                );
        ...
        for server_idx in 0..num_shares {
            ...
            agg[0].verify_sketching_messages(
                server_idx,
                &r_vec,
                &z,
                &z_star,
